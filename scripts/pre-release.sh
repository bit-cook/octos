...
if [ "$SKIP_E2E" = true ]; then
    ...
    if [ "$PROFILE" = "release" ]; then
        OCTOS_BIN="$ROOT/target/release/octos"
    else
        OCTOS_BIN="$ROOT/target/debug/octos"
    fi

    if [ ! -f "$OCTOS_BIN" ]; then
        fail "binary not found at $OCTOS_BIN (run without --skip-build)"
    else
        E2E_DIR=$(mktemp -d)
        trap 'rm -rf "$E2E_DIR"' EXIT

        # 5a. Version output
        if $OCTOS_BIN --version 2>&1 | grep -q "^octos [0-9]"; then
            pass "octos --version"
        else
            fail "octos --version"
        fi
        ...
        # 5b. Help output
        if $OCTOS_BIN --help 2>&1 | grep -q "Usage:"; then
            pass "octos --help"
        else
            fail "octos --help"
        fi
        ...
        if $OCTOS_BIN init 2>&1 && [ -d ".octos" ]; then
        ...
        if $OCTOS_BIN status 2>/dev/null; then
            pass "octos status"
        else
            fail "octos status"
        fi
        ...
        if $OCTOS_BIN skills list 2>/dev/null; then
        ...
        if $OCTOS_BIN cron list 2>/dev/null; then
        ...
        if $OCTOS_BIN channels status 2>/dev/null; then
        ...
        if $OCTOS_BIN completions bash > /dev/null 2>&1; then
        ...
        if $OCTOS_BIN docs 2>&1 | grep -qi "tool\|provider\|Available"; then
            pass "octos docs"
        else
            fail "octos docs"
        fi
        ...
        if $OCTOS_BIN clean 2>/dev/null; then
            pass "octos clean"
        else
            fail "octos clean"
        fi
        ...
        if $OCTOS_BIN auth status 2>&1; then
